...
function(whole_archive_link target lib_dir)
  ...
  elseif(MSVC)
    foreach(LIB ${ARGN})
      string(CONCAT link_flags ${link_flags} "/WHOLEARCHIVE:${LLVM_PROJ_BUILD}/lib/${LIB}.lib ")
    endforeach(LIB)
  else()
...
function(whole_archive_link_onnxmlir target lib_dir)
  get_property(link_flags TARGET ${target} PROPERTY LINK_FLAGS)
  if("${CMAKE_SYSTEM_NAME}" STREQUAL "Darwin")
    set(link_flags "${link_flags} -L${lib_dir}  ")
    foreach(LIB ${ARGN})
  ...
  elseif(MSVC)
    foreach(LIB ${ARGN})
      string(CONCAT link_flags ${link_flags} "/WHOLEARCHIVE:${CMAKE_BINARY_DIR}/${CMAKE_BUILD_TYPE}/lib/${LIB}.lib ")
    endforeach(LIB)
  else()
    set(link_flags "${link_flags} -L${lib_dir} -Wl,--whole-archive,")
    foreach(LIB ${ARGN})
  ...
  set_target_properties(${target} PROPERTIES LINK_FLAGS ${link_flags})
endfunction(whole_archive_link_onnxmlir)


function(whole_archive_link_mlir target)
  whole_archive_link(${target} ${ARGN})
...
function(whole_archive_link_onnx_mlir target)
  ...
  whole_archive_link_onnxmlir(${target} ${ARGN})
